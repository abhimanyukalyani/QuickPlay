...
const base = process.argv[2] ?? "http://localhost:3000";  // matches `npm run serve`
...
// The leaderboard API is a Cloudflare Pages Function, so it does not exist on the
// static export this suite serves — its 404 here is the deployment shape, not a bug.
// Section 8 turns that into a property worth having: the panel must degrade to its
// empty state rather than break the game around it.
const isScoresApi = (u) => /\/api\/scores\//.test(u);
...
async function open(slug, query = "") {
  ...
  const ignore = (u) => isFontHost(u) || isScoresApi(u);
  page.on("requestfailed", (r) => {
    if (!ignore(r.url())) bad.push(`request failed ${r.url()}`);
  });
  page.on("response", (r) => {
    if (r.status() >= 400 && !ignore(r.url())) bad.push(`HTTP ${r.status()} ${r.url()}`);
  });
...
/* ---------- 7. home page and sitemap reflect all nine ---------- */
{
  ...
  if (urls === 11) pass("sitemap has home, leaderboards + nine games", `${urls} urls`);
  else fail("sitemap has home, leaderboards + nine games", `${urls} urls`);
  await page.close();
}

/* ---------- 8. leaderboard wiring, and what happens when the API isn't there ---------- */
{
  // Every ranked game must carry the shared panel and the shared script. Longwave
  // opts out in lib/games.ts: an idle game has no run that ends, so nothing to rank.
  const RANKED = SLUGS.filter((s) => s !== "longwave");
  const missing = [];
  for (const slug of RANKED) {
    const { page } = await open(slug);
    const wired = await page.evaluate(
      () => !!document.getElementById("qp-lb") && typeof window.QPLeaderboard === "object",
    );
    if (!wired) missing.push(slug);
    await page.close();
  }
  if (missing.length) fail("every ranked game is wired to the leaderboard", missing.join(", "));
  else pass("every ranked game is wired to the leaderboard", `${RANKED.length} games`);

  const { page } = await open("longwave");
  const optedOut = await page.evaluate(() => !document.getElementById("qp-lb"));
  if (optedOut) pass("longwave opts out of the leaderboard");
  else fail("longwave opts out of the leaderboard", "found #qp-lb");
  await page.close();

  // With no Pages Function behind it the fetch 404s, and the panel has to say so
  // instead of throwing — this is the path every visitor hits before the first
  // score is posted, and the one a blocked request hits forever.
  const { page: p2, errs } = await open("flipshield");
  const text = await p2.evaluate(async () => {
    const el = document.getElementById("qp-lb");
    await window.QPLeaderboard.renderLeaderboard(el, "flipshield");
    return el.textContent;
  });
  if (/No scores yet/.test(text) && !errs.length) pass("leaderboard degrades when the API is absent", text.trim().slice(0, 40));
  else fail("leaderboard degrades when the API is absent", errs[0] ?? `text: ${text}`);
  await p2.close();

  // The /leaderboards/ index lists the ranked games and leaves the idle one out.
  const p3 = await browser.newPage();
  await p3.goto(`${base}/leaderboards/`, { waitUntil: "domcontentloaded" });
  await p3.waitForTimeout(500);
  const names = await p3.locator("main h2").allInnerTexts();
  const listed = names.map((n) => n.trim().toLowerCase());
  if (listed.length === RANKED.length && !listed.includes("longwave")) {
    pass("leaderboards page lists every ranked game", `${listed.length} boards`);
  } else {
    fail("leaderboards page lists every ranked game", listed.join(", "));
  }
  await p3.close();
}
